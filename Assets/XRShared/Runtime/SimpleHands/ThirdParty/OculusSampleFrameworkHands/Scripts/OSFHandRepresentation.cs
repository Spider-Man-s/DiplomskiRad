...
namespace Fusion.XR.Shared.Core.SimpleHands
{
    /***
     * 
    ...
    public class OSFHandRepresentation : MonoBehaviour, IHandRepresentation, IColoredHandRepresentation, IHandCommandHandler
    {
        ...
        private void Awake()
        {
            if (handMeshRenderer == null) handMeshRenderer = GetComponentInChildren<SkinnedMeshRenderer>(true);
            if (handAnimator == null) handAnimator = GetComponentInChildren<Animator>(true);
            if (handAnimator == null) handAnimator = GetComponentInParent<Animator>(true);
